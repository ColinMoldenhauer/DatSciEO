...
def confusion_matrix_and_classf_metrics(y_true: Iterable, y_pred: Iterable, dataset, 
    ...
    filename: Filename to save confusion matrix image
    '''
    ...
    plt.subplots_adjust(bottom=0.1, top=None if titleConfMatrix else 0.98)
    fig = disp.ax_.get_figure()
    fig.set_figwidth(10)
    fig.set_figheight(8)
    plt.tight_layout()

    with open(os.path.join(output_folder, f"metrics{suffix}.txt"), "w") as f:
        json.dump(metrics, f)
    plt.savefig(os.path.join(output_folder, (filename or titleConfMatrix) + f'{suffix}.png'))